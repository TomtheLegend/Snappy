...
def get_top_cards_by_rating():
    ...
    card_ratings_db = main_app.db.session.execute(sql_card_rating_str).fetchall()
...
def get_top_common_cards_by_current_card_colour():
    sql_card_common_rating_str = 'SELECT name, rating, card_color, card_image FROM' \
                                 ...
                                 ''.format(current_card().card_color,
                                           config['LIMIT'])
    card_common_ratings_db = main_app.db.session.execute(sql_card_common_rating_str).fetchall()
...
def get_all_card_metrics_by_cmc(table, cmc):
    ...
    card_averages_db = main_app.db.session.execute(sql_average_cmc_string).fetchall()
...
def previous_card():
    ...
    if next_id > 0:
        ...
        return main_app.db.session.execute(sql_prev_card_str).first()
    else:
        return None
...
def get_all_distinct_by_cmc(table):
    sql_distinct_cmc_string = 'SELECT DISTINCT cmc FROM' \
                            ' {} '.format(table)
    card_cmcs_db = main_app.db.session.execute(sql_distinct_cmc_string).fetchall()
    ...
    sql_card_colour_str = 'SELECT name, rating, card_rarity, card_image FROM' \
                         ' Card where card_color = \'{}\' ' \
                          'ORDER BY rating DESC LIMIT \'{}\''.format(colour,
                                                                     config['LIMIT'])
    card_colour_db = main_app.db.session.execute(sql_card_colour_str).fetchall()
...
def get_cards_colour_count(colour):
    ...
    solo_colour = main_app.db.session.execute(sql_colour_str).fetchall()
...
def get_cards_colour_count_current_card_rarity(colour):
    ...
    solo_colour_rarity = main_app.db.session.execute(sql_rariry_str).fetchall()
...
def get_card_subtype_count(subtype):
    ...
    solo_type = main_app.db.session.execute(sql_subtype_str).fetchall()
...
def get_previous_card_all_user_ratings():
    ...
    if prev:
        ...
        prev_card_ratings_db = main_app.db.session.execute(all_ratings_sql).fetchall()
...
def get_distinct_supertypes():
    ...
    card_supertypes_db = main_app.db.session.execute(sql_average_supertype_sting).fetchall()
...
def get_supertypes_by_type(supertype):
    sql_average_cmc_sting = 'SELECT rarity, card_colour, card_count FROM ' \
                            'CardSupertypesMetrics WHERE supertypes = \'{}\''.format(supertype)
    # print(sql_average_cmc_sting)
    card_supertypes_db = main_app.db.session.execute(sql_average_cmc_sting).fetchall()
    card_supertypes = [list(card_all) for card_all in card_supertypes_db]
    return card_supertypes
